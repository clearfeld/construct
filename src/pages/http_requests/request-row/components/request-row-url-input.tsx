...
function SingleLineAndVariablePlugin({availableVariables}: {availableVariables: string[]}): null {
  ...
  useEffect(() => {
    return editor.registerNodeTransform(RootNode, (rootNode: RootNode) => {
      ...
      const textContent = rootNode.getAllTextNodes().map(node => node.getTextContent()).join('')
      ...
      if (variableNodes.length > 0) {
        ...
        const lastTextNode: CustomTextNode = {
          ...
          value: textContent.substring(variableNodes[variableNodes.length - 1].end),
          type: 'text',
        }
...
const Placeholder = () => {
  return <div {...stylex.props(styles.placeholder)}>Enter URL or paste text</div>;
}
